...
(defn- app-versioned-stub
  "AppVersioned stub as plain data. Metadata extension resolves the method by
  symbol at call time, so this survives re-evaluating dbos.core in the REPL;
  a `reify` would stay bound to the interface generated by the defprotocol
  that compiled it, and dispatch would fail."
  [version-info]
  (with-meta {} {`core/-get-latest-app-version (fn [_] version-info)}))
...
(deftest run-in-step-ctx-backend-bridge-test
  ...
  (testing "a plain log-fn (no bridge) still runs the body and receives :step/start log"
...
(deftest run-in-step-ctx-emits-step-start-log-test
  (testing "a :step/start log is emitted, with step name carried in trove's :ctx"
    (let [logs (atom [])]
      (binding [trove/*log-fn* (fn [_ns _coords level id payload_]
        ...
        (let [entry (first @logs)]
          (is (= {:id :step/start,
                  :level :info,
                  :payload {:ctx {:workflow/step "fetch"}, :msg "Step start"}}
                 entry)))))))
